...
- **Identity secret keys are Keystore-wrapped, never on disk (decision 10).** The per-user identity **secret** keys — the Ed25519 signing `sk` and the X25519 box `sk` — are device-local secret material under the **same** rule as chat keys: wrapped via Android Keystore directly, **never written to the WebDAV disk and never logged** (stored under a distinct alias so they do not disturb chat keys). The identity **public** keys (Ed25519 `pk`, X25519 `pk`) are **publishable** — they are the material a future directory entry exposes.
