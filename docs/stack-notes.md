...
- **Known gotchas:**
  - **Device-gated tests stay OFF the PR fast path.** `./gradlew connectedAndroidTest` needs an emulator via `reactivecircus/android-emulator-runner`, which requires **KVM hardware acceleration** on the runner (the action's README documents enabling KVM, and it is heavy/slow). PR CI runs the three JVM gates only (`test`, `ktlintCheck`, `lint`); instrumented tests are the documented known CI limitation (architecture decision 8) — run them on demand or in a separate, manually-triggered workflow. Source: <https://github.com/ReactiveCircus/android-emulator-runner#usage>
...
| Secret scan (gitleaks) | `gitleaks detect --source . --redact --exit-code 1` | Enforces **SC21** — no secret material (keys, credentials, passphrases) in the source tree or git history. Pinned gitleaks binary, scans full history (`fetch-depth: 0`), `--redact` so findings are never echoed in plaintext logs; any finding fails the PR. Wired as the `secret-scan` job in `.github/workflows/pr-checks.yml` (parallel to the JVM gates). Source: <https://github.com/gitleaks/gitleaks> |
