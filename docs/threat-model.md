...
Source of decisions: `docs/architecture.md` decisions #1–#11 + `## Security constraints` (SC1–SC16); `docs/protocol/webdav-layout.md` (trust-model recaps §0/§3/§5.1/§8); `.ai-pm/backlog.md` (join-flow / rotation / retention / host-polling notes).
...
- **A1: Private message content** — the plaintext of private (DM and private-group) chats. The product exists to keep this hidden from the disk operator and from outsiders.
...
- **A member offline longer than the retention window losing messages** — the disk holds only a bounded retention window (e.g. 2–4 weeks, configurable); a member offline beyond it loses the oldest messages. Unavoidable without a server and with finite disk (backlog retention model).
- **Cooperative polling-floor enforcement** — the host sets a community-wide minimum polling interval, but enforcement is honest-client: a member can over-poll and hurt the shared 429 budget. Over-polling is hard to attribute under one shared credential. This is a cooperative assumption, not an enforced guarantee (backlog host-polling-floor).
...
- **Physical access to an unlocked device** — once a device is unlocked, the local Room history and in-memory keys are readable. Device-lock and OS-level protections are the user's responsibility; we do not defend an unlocked, in-hand device.
