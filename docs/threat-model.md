...
**What was never assessed:** `[?]` — side-channel analysis of the polling pattern (can the disk operator infer who a member is chatting with from the timing and size of their `changes/` reads?); `[?]` — formal verification of the append-only/content-addressing integrity under concurrent writers; `[?]` — the exact retention window duration and its impact on catch-up reliability under real Yandex 429 rate limits.
