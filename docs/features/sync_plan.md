...
- **Integration contract — on-disk layout:** the shared-log + change-index + window structures and cursor semantics are added to `docs/protocol/webdav-layout.md` (the authoritative interop spec); `Reviewer` blocks a sync implementation that does not reference it.
