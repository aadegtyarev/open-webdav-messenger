...
- `docs/features/chat-directory_plan.md` — the feature that authors §11 (the community chat directory: a second community-scoped collection, reserved `chat-directory/` sibling of `directory/`, §5-envelope-wrapped AEAD-sealed binary versioned-TLV chat descriptors `{chat-id, kind, access, title}`, the signed monotonic supersede coordinate grouped by **chat-id** with content-hash tiebreak, the `group`-only / `dm`-hard-reject / `access ∈ {public,private}` validity rule, and the Depth-1 reject-don't-guess / flat-trust-degradation / DM-drop read). Reuses the §10 framing patterns; does not touch §1–§10.
