...
- **Pruning is implemented — time-based, 14-day default window.** After every successful poll cycle, the `RetentionPruner` scans `log/` and `changes/` and `DELETE`s entries whose embedded order-token timestamp is older than the window (14 days by default). The timestamp is extracted from the first 11 Base32hex characters of the order-token (§4 `ts-millis` segment). Pruning is **disk-only** — local Room history is never touched.
- **Malformed entries are skipped**, not deleted. An entry whose name cannot be parsed as a valid order-token is left in place (it does not match the timestamp decode and is assumed to be not-yet-written or from an incompatible format).
- **Pruning is best-effort.** A listing or deletion failure is silently skipped; the next poll cycle retries. Pruning never affects the poll outcome — a pruner failure does not block cursor advance or message fetch.
- **Change entries are also pruned.** The per-member `changes/<member-index-id>/` entries (§9.2) are pruned by the same time window using the order-token embedded in each change-entry name.
- **No protocol-version bump.** Pruning only `DELETE`s old files and never touches the §2 naming, the §5 framing, or the §9 cursor semantics — it slots in under the existing protocol version 2.
- **Honest limit (already in the threat model / non-goals):** a member offline longer than the window loses the oldest messages from the disk (the disk window is finite and there is no server); the loss is bounded by the window length. Within the window there is **no loss and no double-surfacing** (§9 cursor + §2 message-id dedup guarantee this).
