...
/**
 * An in-memory fake WebDAV disk backing a [okhttp3.mockwebserver.MockWebServer] for the §10 directory
 * tests. Implements just enough of RFC 4918 for the §6/§10.6 access rules the directory uses: `PROPFIND
 * Depth: 1` (collection listing), `GET` (entry body), `PUT` (store), `MKCOL` (collection), `DELETE`.
 * Files are keyed by community-root-relative path (`directory/<entry-name>`). Mirrors `sync/FakeDisk`,
 * rooted at the community-root rather than a chat-root.
 *
 ...
 */
internal class DirectoryFakeDisk(private val communityRoot: String = DirectoryTestSupport.COMMUNITY_ROOT) : Dispatcher() {
