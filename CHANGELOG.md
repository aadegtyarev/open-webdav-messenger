...
- Keyboard overlapping the message input field on newer Android versions
  (Magic OS 10 / Android 14+). Compose bottom bar now explicitly consumes
  IME insets via `imePadding()`.

## [0.22.0] — 2026-06-17

### Added

- Public/private group chats with chat-directory publishing. Public chats
  are auto-discovered by all community members; private chats are invite-only
  and not visible to non-members.
...
- Silent error catches in `refreshMemberNames` and `updateCommunityMetadata`
  now surface to the UI as snackbars instead of failing silently.
