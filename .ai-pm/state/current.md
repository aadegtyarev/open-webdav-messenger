...
- `connectedAndroidTest`: DEVICE-GATED, PENDING — ran `./gradlew connectedDebugAndroidTest`, failed with `com.android.builder.testing.api.DeviceException: No connected devices!` (no emulator/device reachable; decision 8 still open). Instrumented tests compile + are wired; they gate the native AEAD seal/open + Ed25519 sign/verify of a chat descriptor on a device ABI.
- product-map regen (Infrastructure bucket — no contract); release (A/B/C ship gate stays manual).

## Next step

review (pm-plan-checker pass 1).
